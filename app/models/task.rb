class Task < ActiveRecord::Base
  ...
  def self.validate_task(patient, task, location, session_date = Date.today)
    ...
    hiv_reception = patient.encounters.on_date(session_date).most_recent_for_type(art_encounters[1])
    ...
    reception = patient.encounters.on_date(session_date).most_recent_for_type(art_encounters[1]).observations.collect{|r|r.to_s}.join(',') rescue ''
    
    if reception.match(/PATIENT PRESENT FOR CONSULTATION:  YES/i)
      vitals = patient.encounters.on_date(session_date).most_recent_for_type(art_encounters[2])
    ...
    pre_art_visit = patient.encounters.on_date(session_date).most_recent_for_type('PART_FOLLOWUP')

    if pre_art_visit.blank?
      art_visit = patient.encounters.on_date(session_date).most_recent_for_type(art_encounters[4])
    ...
    unless patient.drug_given_before(session_date).blank?
      art_adherance = patient.encounters.on_date(session_date).most_recent_for_type(art_encounters[5])
    ...
    if patient.prescribe_arv_this_visit(session_date)
      art_treatment = patient.encounters.on_date(session_date).most_recent_for_type(art_encounters[6])
  ...
  def self.next_form(location , patient , session_date = Date.today)
    ...
    if location.name.match(/Outpatient/i)
      opd_reception = patient.encounters.on_date(session_date).most_recent_for_type('OUTPATIENT RECEPTION')
    ...
    if (User.current_user.activities & ['Manage Lab Orders', 'Manage Lab Results', 'Manage Sputum Submissions', 'Manage TB Clinic Visits', 'Manage TB Reception Visits', 'Manage TB Registration Visits', 'Manage HIV Status Visits']).any?
    ...
    encounters = GlobalProperty['list.of.clinical.encounters.sequentially'].split(',')
    user_selected_activities = User.current_user.activities.collect(&:upcase).join(',') rescue []
    ...
    encounters.each do | type |
      encounter_available = patient.encounters.on_date(session_date).most_recent_for_type(type)
      reception           = patient.encounters.on_date(session_date).most_recent_for_type('HIV RECEPTION').observations.collect{| r | r.to_s}.join(',') rescue ''
      ...
      case type
        ...
        when 'HIV STAGING'
          if encounter_available.blank? and user_selected_activities.match(/Manage HIV staging visits/i) 
            extended_staging_questions = GlobalProperty['use.extended.staging.questions', 'no'] == 'yes'
        ...
        when 'HIV RECEPTION'
          encounter_art_initial = patient.encounters.on_date(session_date).most_recent_for_type('ART_INITIAL')
          transfer_in = encounter_art_initial.observations.collect{|r|r.to_s.strip.upcase}.include?('HAS TRANSFER LETTER: YES'.upcase)
          hiv_staging = patient.encounters.on_date(session_date).most_recent_for_type('HIV STAGING')
        ...
        when 'ART_INITIAL'
          encounter_art_initial = patient.encounters.on_date(session_date).most_recent_for_type(type)
        ...
        when 'DISPENSING'
          treatment = patient.encounters.on_date(session_date).most_recent_for_type('TREATMENT')
        ...
        when 'TREATMENT'
          encounter_art_visit    = patient.encounters.on_date(session_date).most_recent_for_type('ART VISIT')
  ...
  def self.tb_next_form(location , patient , session_date = Date.today)
    ...
    tb_obs = patient.encounters.on_date(session_date).most_recent_for_type('TB RECEPTION').observations rescue []
    (tb_obs || []).each do | obs |
    ...
    tb_encounters.each do | type |
      ...
      case type
        ...
        when 'VITALS' 
          ...
          unless patient.hiv_status.match(/Positive/i) or patient.tb_status.match(/treatment/i)
            next
          end 

          first_vitals = patient.encounters.on_date(session_date).most_recent_for_type(type)

          unless patient.tb_status.match(/treatment/i) or tb_reception_attributes.include?('Any need to see a clinician: Yes') or patient.hiv_status.match(/Positive/i)
            next
          end

          if patient.tb_status.match(/treatment/i) and not patient.hiv_status.match(/Positive/i) and not first_vitals.blank?
            next
          end

          vitals = patient.encounters.on_date(session_date).most_recent_for_type(type)
        ...
        when 'TB RECEPTION'
          ...
          reception = patient.encounters.on_date(session_date).most_recent_for_type(type)
        ...
        when 'LAB ORDERS'
          reception = Observation.find(:first,:order => "encounter_datetime DESC",:joins => "INNER JOIN encounter USING(encounter_id)",
                                     ...
                                     session_date.to_date, patient.id, EncounterType['TB RECEPTION'].id,
                                     ConceptName['TYPE OF VISIT'].concept_id])

          lab_order = patient.encounters.on_date(session_date).most_recent_for_type(type)
        ...
        when 'SPUTUM SUBMISSION'
          reception = patient.person.observations.first(:order => "encounter_datetime DESC",:joins => "INNER JOIN encounter USING(encounter_id)",
                                     :conditions =>["DATE(encounter_datetime) = ? AND patient_id = ? AND encounter_type = ? AND concept_id = ?",
                                     session_date.to_date, patient.id, EncounterType['TB RECEPTION'].id,
                                     ConceptName['TYPE OF VISIT'].concept_id])

           previous_sputum_sub = patient.encounters.on_date(session_date).most_recent_for_type(type)
        ...
        when 'LAB RESULTS'
          ...
          lab_result = patient.encounters.on_date(session_date).most_recent_for_type(type)
        ...
        when 'TB CLINIC VISIT'
          obs_ans = patient.person.observations.on_date(session_date).with_concept('ANY NEED TO SEE A CLINICIAN').to_s.strip.squish rescue nil
          ...
          if not obs_ans.blank?
            ...
            if obs_ans.match(/ANY NEED TO SEE A CLINICIAN: YES/i)
              tb_visits =patient.encounters.on_date(session_date).most_recent_for_type('TB VISIT')
          ...
          clinic_visit = patient.encounters.on_date(session_date).most_recent_for_type(type)
        ...
        when 'TB_INITIAL'
          ...
          tb_initial = patient.encounters.most_recent_for_type(type)
        ...
        when 'ART_INITIAL'
          ...
          enrolled_in_hiv_program = Concept.find(patient.person.observations.with_concept('Patient enrolled in IMB HIV program').last.value_coded).concept_names.map(&:name).first.try :upcase
          ...
          art_initial = patient.encounters.most_recent_for_type(type)
        ...
        when 'HIV STAGING'
          ...
          enrolled_in_hiv_program = Concept.find(Observation.with_concept('Patient enrolled in IMB HIV program').last(:conditions => {:person_id => patient.id}).value_coded).concept_names.map(&:name).first.try :upcase
          ...
          next unless patient.hiv_status.match(/Positive/i)
          hiv_staging = patient.encounters.most_recent_for_type(type)
        ...
        when 'TB REGISTRATION'
          ...
          tb_registration = patient.encounters.most_recent_for_type(type)
        ...
        when 'TB VISIT'
          if patient.child? or patient.hiv_status.match(/Positive/i)
            clinic_visit = patient.encounters.most_recent_for_type('TB CLINIC VISIT')
          ...
          tb_registration = patient.encounters.most_recent_for_type('TB REGISTRATION')

          tb_followup = patient.encounters.on_date(session_date).most_recent_for_type(type)
        ...
        when 'ART VISIT'  
          ...
          clinic_visit = patient.encounters.on_date(session_date).most_recent_for_type('TB CLINIC VISIT')
          ...
          art_visit = patient.encounters.on_date(session_date).most_recent_for_type(type)
        ...
        when 'TB ADHERENCE'
          ...
          tb_adherence = patient.encounters.on_date(session_date).most_recent_for_type(type)
        ...
        when 'ART ADHERENCE'
          ...
          art_adherence = patient.encounters.on_date(session_date).most_recent_for_type(type)
        ...
        when 'TREATMENT' 
          tb_treatment_encounter = patient.encounters.recent.on_date(session_date).typed(type).width_concept('TB regimen type').first

          encounter_tb_visit = patient.encounters.on_date(session_date).most_recent_for_type('TB VISIT')
          ...
          if not prescribe_drugs 
            encounter_tb_clinic_visit =patient.encounters.on_date(session_date).most_recent_for_type('TB CLINIC VISIT')
          ...
          art_treatment_encounter = patient.encounters.recent.on_date(session_date).typed(type).width_concept('ARV regimen type').first
          ...
          if not prescribe_drugs
            encounter_pre_art_visit = patient.encounters.recent.on_date(session_date).typed('PART_FOLLOWUP')
        ...
        when 'DISPENSING'
          treatment = patient.encounters.recent.on_date(session_date).typed('TREATMENT')
          encounter = patient.encounters.recent.on_date(session_date).typed('DISPENSING')
  ...
  def self.need_art_enrollment(task,patient,location,session_date,user_selected_activities,reason_for_art)
    ...
    enrolled_in_hiv_program = Concept.find(patient.person.observations.with_concept('Patient enrolled in IMB HIV program').last.value_coded).concept_names.map(&:name).first.upcase rescue nil

    return unless enrolled_in_hiv_program == 'YES'

    #return if not reason_for_art.upcase == 'UNKNOWN' and not reason_for_art.blank?

    art_initial = patient.encounters.recent.typed('ART_INITIAL').first
    ...
    hiv_staging = patient.encounters.recent.typed('HIV STAGING').first
    ...
    pre_art_visit = patient.encounters.recent.typed('PART_FOLLOWUP').first
    ...
    art_visit = patient.encounters.recent.typed('ART VISIT').first
    ...
    treatment_encounter = patient.encounters.recent.typed('TREATMENT').with_concept('ARV regimen type').first
  ...
  def self.next_lab_encounter(encounter = nil , session_date = Date.today)
    ...
    case encounter.name.upcase
      when 'LAB ORDERS' 
        ...
        sputum_sub = patient.encounters.typed(type).first(:include => :observations,
               :conditions => {:accession_number => encounter.observations.map(&:accession_number).compact})

        return sputum_sub.blank? ? type : sputum_sub
      when 'SPUTUM SUBMISSION'
        type = EncounterType.find_by_name('LAB RESULTS').id
        lab_results = patient.encounters.typed(type).first(:include => :observations,
               :conditions => {:accession_number => encounter.observations.map(&:accession_number).compact})

        type = EncounterType.find_by_name('LAB ORDERS').id
        lab_order = patient.encounters.typed(type).first(:include => :observations,
               :conditions => {:accession_number => encounter.observations.map(&:accession_number).compact})

        return lab_order if lab_results.blank? and not lab_order.blank?
        return lab_results unless lab_results.blank?
      when 'LAB RESULTS'
        type = EncounterType.find_by_name('SPUTUM SUBMISSION').id
        sputum_sub = patient.encounters.typed(type).first(:include => :observations,
               :conditions => {:accession_number => encounter.observations.map(&:accession_number).compact})

        return sputum_sub unless sputum_sub.blank?
    end
  end


  def self.checks_if_vitals_are_need(patient , session_date, task , user_selected_activities)
    first_vitals = patient.encounters.most_recent_for_type('VITALS')

    if first_vitals.blank?
      encounter  = patient.encounters.most_recent_for_type('LAB ORDERS')
      sup_result = self.next_lab_encounter(encounter, session_date)
      reception  = patient.encounters.on_date(session_date).most_recent_for_type('TB RECEPTION')
    ...
    vitals = patient.encounters.on_date(session_date).most_recent_for_type('VITALS')
  ...
  def self.checks_if_labs_results_are_avalable_to_be_shown(patient , session_date , task)
    lab_result       = patient.encounters.on_date(session_date).most_recent_for_type('LAB RESULTS')
    give_lab_results = patient.encounters.on_date(session_date).most_recent_for_type('GIVE LAB RESULTS')
