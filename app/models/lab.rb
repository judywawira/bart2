class Lab < ActiveRecord::Base
  ...
  def self.results(patient)
    patient_ids = patient.identifier_values
...
GROUP BY short_name ORDER BY m.short_name", patient_ids]).collect do |result|
      [ result.short_name,
        result.TestName,
        result.Range,
        result.TESTVALUE,
...
AND s.deleteyn = 0
AND s.attribute = 'pass'
ORDER BY DATE(TESTDATE) DESC", patient_ids, type]).collect do |result|
