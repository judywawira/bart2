...
class User < ActiveRecord::Base
  ...
  def authenticated?(plain)
    encrypt(plain, salt) == password or
        Digest::SHA1.hexdigest("#{plain}#{salt}") == password or
        Digest::SHA512.hexdigest("#{plain}#{salt}") == password
  end
  
  def admin?
    (user_roles.map(&:role) & ['Informatics Manager', 'System Developer', 'Superuser']).any?
  end
